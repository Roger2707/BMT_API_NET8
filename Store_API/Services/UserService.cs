...
namespace Store_API.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<Result<List<UserAddressDTO>>> GetUserAddresses(string userName)
        {
            var user = await _userManager.FindByNameAsync(userName);

            string query = @" SELECT Id, City, District, Ward, PostalCode, StreetAddress, Country FROM UserAddresses WHERE UserId = @UserId ";
        ...
        public async Task<Result<dynamic>> UpsertUserAddresses(string userName, UserAddressDTO userAddressDTO)
        {
            ...
            try
            {
                ...
                if (id != 0)
                {
                    string query = @" UPDATE UserAddresses 
                                        ...
                                        WHERE Id = @Id
                                    ";
                    var p = userAddressDTO;
                    await _dapperService.Execute(query, p);
                }
                else
                {
                    string query = @" INSERT INTO UserAddresses VALUES(@Id, @UserId, @City, @District, @Ward, @StreetAddress, @PostalCode, @Country) ";
                ...
                return Result<dynamic>.Success(id == 0 ? "Create new address successfully !" : "Update address successfully !");
            }
